...
def dodeps(path):
    for (dgid, daid, dver, scope) in extractdeps(path):
        print path, dgid, daid, dver, scope
...
def main():
    ...
    dodeps('tests/jsunit-1.3.pom')
    dodeps('tests/xxl-1.0.pom')
    dodeps('tests/HTTPClient-0.3-3.pom')
    dodeps('tests/tranql-connector-derby-client-local-1.0.pom')
